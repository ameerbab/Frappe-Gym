...
def on_submit_events(doc, method=None):
    ...
    for row in doc.items:
        ...
        if itemwise_projected_qty.get(key) and itemwise_projected_qty.get(key) < 0:
            po_items.setdefault(key, frappe._dict({"projected_qty": flt(abs(itemwise_projected_qty.get(key)))}))
...
def get_projected_qty_of_items(doc) -> dict:
    ...
    item_details = {}
    data = frappe.get_all("Bin", fields=["item_code", "warehouse", "projected_qty"], filters={"item_code": ("in", items), "warehouse": ("in", warehouses)})
...
def set_default_supplier(po_items):
    ...
    supplier_dict = {}
    data = frappe.get_all("Item Default", fields = ["parent", "default_supplier"], filters={"parent": ("in", items)})
